...
###################################################
##==== CatBoost hyperparameters search space ====##
###################################################
...
def _catboost_subsample(name):
    return hp.uniform(name, 0.5, 1)

def _catboost_colsample_bytree(name):
    return hp.uniform(name, 0.5, 1)

def _catboost_colsample_bylevel(name):
    return hp.uniform(name, 0.5, 1)

def _catboost_reg_alpha(name):
    return hp.loguniform(name, np.log(0.0001), np.log(1)) - 0.0001
...
def _catboost_hp_space(name_func,
    iterations=None,
    learning_rate=None,
    depth=None,
    l2_leaf_reg=None,
    ...
    hp_space = dict(
        ...
        reg_lambda=(_catboost_reg_lambda(name_func('reg_lambda'))
                    if reg_lambda is None else reg_lambda),
        scale_pos_weight=(_catboost_scale_pos_weight(name_func('scale_pos_weight'))
                    if scale_pos_weight is None else scale_pos_weight),
        bagging_temperature=(_catboost_bagging_temp(name_func('bagging_temperature'))
...
########################################################
##==== CatBoost classifier/regressor constructors ====##
########################################################
def catboost_classification(name, **kwargs):
    '''
    Return a pyll graph with hyperparameters that will construct
